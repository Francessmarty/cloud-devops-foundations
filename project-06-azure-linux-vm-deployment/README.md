...
## Gateway Options (VPN Gateway / ExpressRoute)

This project used a VNet + subnet for VM networking. In real enterprise setups, VNets are often connected to on-prem or other networks using gateway services such as:

- VPN Gateway (IPsec/IKE): Encrypted tunnel over the public internet. Best for quick hybrid connectivity, dev/test, or smaller workloads. Typically uses route propagation (BGP optional) and works with UDRs/route tables.
- ExpressRoute: Private connectivity (not over the public internet) via a provider circuit. Best for production-grade hybrid connectivity where you need predictable performance, higher reliability, and consistent latency.

- Routing/NAT note: Even without a gateway, outbound traffic patterns are controlled via routing (system routes + UDRs) and can be centralized using services like a *NAT Gateway* for predictable outbound IPs.
